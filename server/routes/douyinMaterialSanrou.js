...
if (!isProduction) {
  // 开发环境使用项目内路径
  CONFIG_FILE_PATH = path.join(__dirname, '../data/douyin-material-config_sanrou.json')
}

// 启动时打印配置文件路径，方便确认环境变量是否生效
console.log('📁 抖音号素材匹配配置文件路径(散柔):', CONFIG_FILE_PATH)
console.log('📁 环境: ', isProduction ? '生产环境' : '开发环境')
console.log(
  '📁 环境变量 DOUYIN_MATERIAL_CONFIG_SANROU_PATH:',
  process.env.DOUYIN_MATERIAL_CONFIG_SANROU_PATH || '未设置'
)
...
/**
 * 获取所有抖音号素材匹配配置（散柔）
 * GET /api/douyin-material-sanrou/config
 */
router.get('/config', async ctx => {
  try {
    ...
    console.error('读取抖音号素材匹配配置(散柔)失败:', error)
    ctx.status = 500
    ctx.body = {
      code: -1,
      message: '读取配置失败',
...
/**
 * 获取格式化的抖音号素材配置（散柔） - 供外部服务使用
 * GET /api/douyin-material-sanrou/formatted
...
/**
 * 添加抖音号素材匹配规则（散柔）
 * POST /api/douyin-material-sanrou/config
...
/**
 * 更新抖音号素材匹配规则（散柔）
 * PUT /api/douyin-material-sanrou/config/:id
 * Body: { douyinAccount?, douyinAccountId?, materialRange? }
 */
...
/**
 * 删除抖音号素材匹配规则（散柔）
 * DELETE /api/douyin-material-sanrou/config/:id
 */
